...
beforeAll(() => {
  // 테스트를 돌리는 기본 시스템 날짜를 2026년 6월 9일(화요일)로 고정
  jest.useFakeTimers();
  jest.setSystemTime(new Date(2026, 5, 9));
});
...
describe("월간 모드 검증", () => {
  test("type이 month인 경우 calendarSlides의 id 형식이 yyyy-MM 포맷으로 생성된다", () => {
    const { result } = renderHook(() => useCalendar(new Date(), "month"));
...
describe("useCalendar 훅 테스트", () => {
  describe("주간 모드 검증", () => {
    test("type이 week인 경우 calendarSlides의 id 형식이 주간 연도 포맷(yyyy-II)으로 생성된다", () => {
      const { result } = renderHook(() => useCalendar(new Date(), "week"));
